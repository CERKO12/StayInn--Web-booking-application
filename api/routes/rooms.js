...
router.post("/:hotelId", async (req, res) => {
    ...
    try {
        ...
        try {
            await myDB.updateHotelById(hotelId, {$push : {rooms: savedRoom.insertedId}});
        } catch(err) {
...
router.put("/:id", async (req, res) => {
    try {
        const updatedRoom = await myDB.updateRoomById(req.params.id, {$set: req.body});
...
router.put("/availability/:hotelId/:number", async (req, res) => {
    try {
        ...
        await dates.map((date) => {
            myDB.updateRoomAvailability(req.params.hotelId, req.params.number, {$push: 
